...
%
% Deliberately steep rather than merely non-axis-aligned. tan(89.95 deg) is 1145.9,
% which slope-intercept still handles cleanly here - the line passes through the
% origin so the intercept stays at machine zero and conditioning is fine. What it
% pins down is the consumer side: PMKSWeb clamped any slope above 1000 to
% Number.MAX_VALUE and fell through to a branch that holds x constant, which is
% right for an exactly vertical guide and wrong for a steep one. Measured against
% this geometry it left the joint 4.0e-3 off its own track.
